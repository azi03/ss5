...
inline void S5DebugUpstreamInfoFrom( pid_t pid, struct _SS5RequestInfo ri, char *from ) {
  char logString[256];

  struct in_addr in;

  in.s_addr=ri.upDstAddr;
  snprintf(logString,256 - 1,"[%u] [DEBU] %s UpstreamInfo->DstAddr       %s.",pid, from, (char *)inet_ntoa(in));     LOGUPDATE()
  snprintf(logString,256 - 1,"[%u] [DEBU] %s UpstreamInfo->DstPort       %5d.",pid, from, ri.upDstPort);               LOGUPDATE()
}

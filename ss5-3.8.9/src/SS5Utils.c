  ...
  while( fscanf(S5ConfFile,"%255s",confString)!=EOF ) {
     ...
     */
      ...
      if( _tmp_NProxyList < MAXPROXYLIST ) {
        ...
        if (SS5SocksOpt.Verbose ) {
            snprintf(logString,256 - 1,"[VERB] loading proxy/noproxy: %25s %20s %20s %10s %1s.", pcf.dstAddr, pcf.dstPort, pcf.pxyAddr, pcf.pxyPort, pcf.upSocksV);
            LOGUPDATE()
        ...
        if( inet_aton(pcf.dstAddr,&in) == 0 ) {
        	SS5Modules.mod_socks5.AddProxy(OFFLINE,STREQ(confString,"proxy",sizeof("proxy") - 1)?PROXY:NOPROXY,S5StrHash(pcf.dstAddr),S5GetRange(pcf.dstPort),inet_addr(pcf.pxyAddr),atoi(pcf.pxyPort),32-dstMask,socksver);
        }
        else {
        	in.s_addr=inet_addr(pcf.pxyAddr);
        	SS5Modules.mod_socks5.AddProxy(OFFLINE,STREQ(confString,"proxy",sizeof("proxy") - 1)?PROXY:NOPROXY,inet_network(pcf.dstAddr),S5GetRange(pcf.dstPort),inet_addr(pcf.pxyAddr),atoi(pcf.pxyPort),32-dstMask,socksver);
        }

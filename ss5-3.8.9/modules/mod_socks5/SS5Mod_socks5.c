    ...
    if( S5ProxyList[index]!= NULL ) {
      ...
      do {
        if( (node->DstAddr == n_da) && (node->Mask == nm) && (node->DstPort == dp) ) {
          ...
          if( node->Type == PROXY ) {
        	  if( DEBUG() ) {
        		  S5DebugUpstreamInfoFrom(pid, *ri, "GetProxy With port");
				}
        	  return OK;
          }
          return ERR_NOPROXY;
    ...
    if( S5ProxyList[index]!= NULL ) {
      ...
      do {
        if( (node->DstAddr == n_da ) && (node->Mask == nm) && (dp >= node->DstRangeMin) && (dp <= node->DstRangeMax) ) {
          ...
          if( node->Type == PROXY ) {
        	  if( DEBUG() ) {
        		  S5DebugUpstreamInfoFrom(pid, *ri, "GetProxy With all Port");
				}
            return OK;
          }
          return ERR_NOPROXY; 
...
UINT AddProxy(UINT ctx,UINT type, ULINT da, ULINT dp, ULINT pa, UINT pp, UINT mask, UINT socksver)
{
  ...
  SS5RequestInfo.upDstAddr = pa;
  SS5RequestInfo.upDstPort = pp;

  if( DEBUG() ) {
	  S5DebugUpstreamInfoFrom(pid, SS5RequestInfo, "AddProxy");
  }
